...
class EventsPaymentPage extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    if(content.data.hasOwnProperty("detailsOnly")) {
      this.setState({ loaded: false, redirectDetails: true });
      return;
    }
  ...
  render () {
    if(!this.state.loaded) {
      ...
      if(this.state.redirectDetails) {
        return (
          <Redirect to={`/events/bookings/free/${this.state.id}`} />
        );
      }
    ...
    const { event, group, guestTickets, type, bookingCloses, totalCost, clientSecret, paidNow, extraDetailsGiven } = this.state;
