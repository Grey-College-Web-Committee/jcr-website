...
class ApproveRow extends React.Component {
  constructor(props) {
    super(props);
  ...
  approve = async () => {
    ...
    try {
      await api.post("/auth/action", { id: this.props.record.id, approved: true });
    } catch (error) {
  ...
  deny = async () => {
    ...
    try {
      await api.post("/auth/action", { id: this.props.record.id, approved: false });
    } catch (error) {
  ...
  render () {
    const { id, username, firstName, surname, year, createdAt } = this.props.record;

    return (
      <tr className={`text-center border-b border-gray-400`}>
        <td className="p-2 border-r border-gray-400">{id}</td>
        <td className="p-2 border-r border-gray-400">{username}</td>
        <td className="p-2 border-r border-gray-400">{firstName}</td>
