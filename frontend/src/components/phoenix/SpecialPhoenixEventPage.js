...
class SpecialPhoenixEventPage extends React.Component {
  ...
  renderAdditional = () => {
    ...
    return (
      <div className="w-full">
        <p className="py-1 text-xl font-semibold text-left">Total Price: £65</p>
        <div className="pt-2 pb-2 border-b-2">
          <label htmlFor="guestNameAdd" className="flex flex-row justify-start text-xl font-semibold">Guest Name</label>
          <span className="flex flex-row justify-start text-sm mb-2">({255 - this.state.guestNameAdd.length} characters remaining)</span>
          <input
            type="text"
            name="guestNameAdd"
            value={this.state.guestNameAdd}
            onChange={this.onInputChange}
            className="border w-full rounded py-1 px-2 focus:outline-none focus:ring-2 disabled:opacity-50 focus:ring-gray-400"
            disabled={this.state.disabled}
            autoComplete=""
  ...
  onPaymentSuccessAdd = () => {
    this.setState({ paymentStAdd: 2 });
  }

  confirmAddBooking = async () => {
    this.setState({ disabled: true });
    let { guestNameAdd, guestDietAdd } = this.state;

    let result;

    try {
      result = await api.post("/phoenix/additional", { guestName: guestNameAdd, guestDiet: guestDietAdd });
    } catch (error) {
      alert(error.response.data.error);
      return;
    }
    ...
    this.setState({ disabled: false, totalCost, clientSecret, paymentStAdd: 1 });
  }

  canSubmitAdd = () => {
    return (this.state.guestNameAdd !== null && this.state.guestNameAdd !== "") &&
           (this.state.guestDietAdd !== null && this.state.guestDietAdd !== "select");
  }
  ...
  render () {
    ...
    if(this.state.booked) {
      if(!this.state.isMember) {
        return (
          <div className="flex flex-col justify-start">
            <div className="md:w-3/5 container mx-auto text-center p-4">
              <h1 className="font-semibold text-5xl pb-4">Phoenix Festival</h1>
              <p className="py-1">You have already booked a ticket for this event. Please check your email for confirmation.</p>
              <p className="py-1">JCR members can purchase additional guest tickets for £65 each.</p>
            </div>
          </div>
        )
      }

      if(this.state.paymentStAdd === 2) {
        return (
          <div className="flex flex-col justify-start">
            <div className="md:w-3/5 container mx-auto text-center p-4">
              ...
              <div className="flex-1 mr-0 md:mr-2 p-2 text-left border border-red-900 mt-2">
                ...
                <div className="w-full text-base py-4 align-middle">
                  <CheckoutForm
                    ...
                    onSuccess={this.onPaymentSuccessAdd}
      ...
      if(this.state.paymentStAdd === 0) {
        return (
          <div className="flex flex-col justify-start">
            <div className="md:w-3/5 container mx-auto text-center p-4">
              <h1 className="font-semibold text-5xl pb-4">Phoenix Festival</h1>
              <p className="text-left py-1">You have already booked a ticket for this event. Please check your email for confirmation.</p>
              <p className="text-left py-1">You can purchase additional guest tickets for £65.</p>
              { this.renderAdditional() }
              <button
                onClick={this.confirmAddBooking}
                ...
                disabled={this.state.disabled || !this.canSubmitAdd()}
              >Confirm Guest and Pay</button>
            </div>
          </div>
        )
      }
