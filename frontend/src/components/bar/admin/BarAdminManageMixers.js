...
class BarAdminManageMixers extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    try {
      content = await api.get("/bar/admin/mixers");
    } catch (error) {
    ...
    this.setState({ loaded: true, mixers: content.data.mixers });
  }

  createNewMixer = async () => {
    if(!this.canSubmit()) {
            ...
            <fieldset>
              ...
              <div className="pt-2 pb-2 border-b-2">
                <button
                  ...
                  onClick={this.createNewMixer}
                >Create New Mixer</button>
              </div>
            </fieldset>
          </div>
...
export default BarAdminManageMixers;
