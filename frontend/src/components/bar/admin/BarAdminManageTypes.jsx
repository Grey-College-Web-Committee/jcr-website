...
class BarAdminManageTypes extends React.Component {
  ...
  createNewType = async () => {
    ...
    const { name, allowsMixer, allowsCordial } = this.state;

    let result;

    try {
      result = await api.post("/bar/admin/type", {
        name, allowsMixer, allowsCordial
      });
    } catch (error) {
    ...
    this.setState({ disabled: false, name: "", allowsMixer: false, allowsCordial: false, types });
  }
  ...
  canSubmit = () => {
    return (
      ...
      (this.state.allowsCordial !== undefined && this.state.allowsCordial !== null)
    );
  }
        ...
        <div className="container mx-auto text-center p-4 md:w-3/5 w-full">
          ...
          <div>
            ...
            <fieldset>
              ...
              <div className="pt-2 pb-2 border-b-2 flex flex-row items-center">
                <label htmlFor="allowsCordial" className="flex flex-row justify-start text-xl font-semibold flex-1 items-center">Allows Cordial?</label>
                <div className="flex flex-col items-center justify-center ml-2">
                  <input
                    type="checkbox"
                    name="allowsCordial"
                    checked={this.state.allowsCordial}
                    onChange={this.onInputChange}
                    className="p-2 h-8 w-8 align-middle mx-auto rounded border border-black focus:outline-none focus:ring-2 focus:ring-gray-400 disabled:opacity-50"
                    disabled={this.props.disabled}
                    autoComplete=""
            ...
            <table className="mx-auto border-2 text-left border-red-900 w-full mt-4">
              <thead className="bg-red-900 text-white">
                <tr>
                  ...
                  <th className="p-2 font-semibold">Allows Cordial</th>
