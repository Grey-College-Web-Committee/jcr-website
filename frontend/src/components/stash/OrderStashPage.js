...
import SelectBread from './SelectClothing';
...
class OrderStashPage extends React.Component {
  ...
  passUpBread = (clothing) => {
    this.setState({ clothing: Number(clothing) }, this.calculateCost);
  }
  ...
  placeOrder = async () => {
    ...
    try {
      query = await api.post("/stash/order", orderDetails);
    } catch (error) {
  ...
  render () {
    ...
    if(!config.debug && this.state.stashPurchaseDisabled) {
      return (
        <React.Fragment>
          <h1>We are Not Currently Accepting Stash Orders</h1>
