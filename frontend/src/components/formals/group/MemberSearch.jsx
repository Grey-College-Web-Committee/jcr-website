...
class MemberSearch extends React.Component {
  ...
  searchForUser = async () => {
    ...
    try {
      result = await api.get(`/formals/search/${this.props.formalId}/${this.state.username}`);
    } catch (error) {
        ...
        <fieldset>
          ...
          }
          {
            this.state.result === null ? null : (
              <p className="py-1">Added {this.state.result.name} to your group!</p>
            )
          }
        </fieldset>
      </div>
...
MemberSearch.propTypes = {
  ...
  formalId: PropTypes.string.isRequired,
